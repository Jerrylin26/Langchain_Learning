...
text_splitter = CharacterTextSplitter(
    separator="\n\n",
    chunk_size=300, # 最大長度限制
    chunk_overlap = 50, #重疊處
    length_function = len
)

# 沒機會累積到 chunk_size 以上，因此 overlap 沒有被觸發。
# 維持語意連續性
#段落短 → chunk 本身就完整 → 沒有多段落累積 → 重疊不需要
#段落長或多段落累積 → chunk 超過 chunk_size → overlap 才會回頭保留上下文
